...
for iteracao = 1:k_fold
    ...
    for cont = 1:quant_teste
        ...
        vetor_qda = zeros(6,1);
        vetor_qda(1) = log(det(matriz_cov_classe1)) + (((X - vetor_media_classe1.').')*inv(matriz_cov_classe1)*(X - vetor_media_classe1.')) - (2*log(probabilidade_priori(1)));
        vetor_qda(2) = log(det(matriz_cov_classe2)) + (((X - vetor_media_classe2.').')*inv(matriz_cov_classe2)*(X - vetor_media_classe2.')) - (2*log(probabilidade_priori(2)));
        vetor_qda(3) = log(det(matriz_cov_classe3)) + (((X - vetor_media_classe3.').')*inv(matriz_cov_classe3)*(X - vetor_media_classe3.')) - (2*log(probabilidade_priori(3)));
        vetor_qda(4) = log(det(matriz_cov_classe4)) + (((X - vetor_media_classe4.').')*inv(matriz_cov_classe4)*(X - vetor_media_classe4.')) - (2*log(probabilidade_priori(4)));
        vetor_qda(5) = log(det(matriz_cov_classe5)) + (((X - vetor_media_classe5.').')*inv(matriz_cov_classe5)*(X - vetor_media_classe5.')) - (2*log(probabilidade_priori(5)));
        vetor_qda(6) = log(det(matriz_cov_classe6)) + (((X - vetor_media_classe6.').')*inv(matriz_cov_classe6)*(X - vetor_media_classe6.')) - (2*log(probabilidade_priori(6))); 
...
disp("QDA - Média das Taxas de Acurácia em %")
