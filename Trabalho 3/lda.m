...
for iteracao = 1:k_fold
    ...
    matriz_cov_agregada = zeros(colunas_base-1, colunas_base-1);
    for i = 1: colunas_base-1
        for j = 1: colunas_base-1
            matriz_cov_agregada(i,j) = (matriz_cov_classe1(i,j)+matriz_cov_classe2(i,j)+matriz_cov_classe3(i,j)+matriz_cov_classe4(i,j)+matriz_cov_classe5(i,j)+matriz_cov_classe6(i,j))/6;
        end
    end
    
    %% LDA
    % Equação: 
    % min i: (X - Ui)^T . M_agregada^-1 . (X - Ui)
    ...
    for cont = 1:quant_teste
        ...
        vetor_lda = zeros(6,1);
        vetor_lda(1) = (((X - vetor_media_classe1.').')*inv(matriz_cov_agregada)*(X - vetor_media_classe1.'));
        vetor_lda(2) = (((X - vetor_media_classe2.').')*inv(matriz_cov_agregada)*(X - vetor_media_classe2.'));
        vetor_lda(3) = (((X - vetor_media_classe3.').')*inv(matriz_cov_agregada)*(X - vetor_media_classe3.'));
        vetor_lda(4) = (((X - vetor_media_classe4.').')*inv(matriz_cov_agregada)*(X - vetor_media_classe4.'));
        vetor_lda(5) = (((X - vetor_media_classe5.').')*inv(matriz_cov_agregada)*(X - vetor_media_classe5.'));
        vetor_lda(6) = (((X - vetor_media_classe6.').')*inv(matriz_cov_agregada)*(X - vetor_media_classe6.'));
        
        [~, posicao] = min(vetor_lda);  % Menor posição é o rótulo estimado
...
disp("LDA - Média das Taxas de Acurácia em %")
