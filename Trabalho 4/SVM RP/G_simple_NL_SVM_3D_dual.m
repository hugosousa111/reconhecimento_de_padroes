...
XNL = [X.^2; X(1,:).*X(2,:); X(1,:).*X(3,:); X(2,:).*X(3,:); X(1,:).*X(2,:).*X(3,:)];

% Gráfico de dispersão (parcial) dos dados tranformados
figure,  plot3(XNL(1,1:N/2),XNL(2,1:N/2),XNL(3,1:N/2),'bd')
hold on, plot3(XNL(1,N/2+1:end),XNL(2,N/2+1:end),XNL(3,N/2+1:end),'ro')
grid on
axis([-2 2 -2 2 -2 2])
