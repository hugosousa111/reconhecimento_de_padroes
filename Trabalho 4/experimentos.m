...
matriz_c_RBF = zeros(2, 2, n_experimentos_RBF);
cont = 1;
...
for ex_c = 1: length(C)
    for ex_g = 1: length(gamma)
        ...
        [m, s, m_c] = pca_svm(base,iteracoes, rth, C(ex_c), gamma(ex_g), kernel_rbf);
        mean_ac_RBF(cont) = m;
        std_ac_RBF(cont) = s;
        matriz_c_RBF(:, :, cont) = m_c;
        fprintf('\n\n------------------------------------------------\n\n')
        cont = cont+1;
    end
end
...
matriz_c_linear = zeros(2, 2, n_experimentos_linear);
...
for ex_c = 1: length(C)
    ...
    [m, s, m_c] = pca_svm(base,iteracoes, rth, C(ex_c), gamma(1), kernel_linear);
    mean_ac_linear(ex_c) = m;
    std_ac_linear(ex_c) = s;
    matriz_c_linear(:, :, ex_c) = m_c;
    fprintf('\n\n------------------------------------------------\n\n')
end
...
for ex_c = 1: length(C)
    ...
    fprintf('Média das Taxas de Acurácia(em porcentagem) => %f\n', mean_ac_linear(ex_c))
    fprintf('Desvio padrão das Taxas de Acurácia => %f\n', std_ac_linear(ex_c))
    fprintf('Matriz de confusão (Média) => \n  %0.2f   %0.2f\n  %0.2f   %0.2f ',matriz_c_linear(:, :, ex_c))
    fprintf('\n\n---------------------------------\n\n')
end