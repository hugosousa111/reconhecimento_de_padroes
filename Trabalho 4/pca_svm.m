function [media_ac,std_ac, media_matriz_conf] = pca_svm(base,iteracoes,rth, C, gamma, kernel)
    %% Roda varias vezes e tira a média das acurácias
    vetor_acuracia = zeros(iteracoes,1);
    media_matriz_conf = zeros(2,2);
    ...
    for iteracao = 1:iteracoes
        ...
        fprintf('\nMatriz de Confusão => \n  %d   %d\n  %d   %d',matriz_confusao)
        media_matriz_conf = media_matriz_conf+matriz_confusao;
    ...
    media_matriz_conf = media_matriz_conf/iteracoes;
    %disp(media_matriz_conf)
